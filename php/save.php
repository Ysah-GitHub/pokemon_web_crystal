<?php
  ...
  $req = $bdd->prepare("SELECT name, password, save FROM user WHERE name = :name");
  $req->execute(array('name' => $name));
  if ($req->rowCount() == 1) {
    $password_hashed = $req->fetch()["password"];
    if (password_verify($password, $password_hashed)) {
      $req = $bdd->prepare("UPDATE user SET save = :save WHERE name = :name");
      $req->execute(array('save' => $save, 'name' => $name));
      echo "Syncronisation réussi"; return;
    }
    else {
    ...
    $req = $bdd->prepare("INSERT INTO user(name, password, save) VALUES(:name, :password, :save)");
    $req->execute(array('name' => $name, 'password' => password_hash($password, PASSWORD_DEFAULT), 'save' => $save));
    echo "Syncronisation réussi, nouvelle entrée créée"; return;
  }
